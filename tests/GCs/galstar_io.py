...
def load_stacked_bins_sparse(fname, p0=-1., selection=None, verbose=False):
	'''
	Load binned probability density functions (pdfs) from a sparse,
	uncompressed galstar bin output file, stacking output.
	...
	Output:
		bounds[4] = [x_min, x_max, y_min, y_max]
		bin_data (numpy float64 array) = p(x, y), where n is the index of
		                                     the star, and x and y are the
		                                     axes (DM and Ar, for example)
	'''
	...
	# Create an empty array to populate
	N_files_sel = sel_sorted.size;
	bin_data = np.zeros((bin_width[0], bin_width[1]), dtype=np.float64)
	tmp_data = np.zeros((bin_width[0], bin_width[1]), dtype=np.float64)
	obj_id = np.empty(N_files_sel, dtype=np.uint64)
	lb = np.empty((N_files_sel, 2), dtype=np.float64)
	N_empty = 0
	...
	code = """
			...
			if((int)sel_sorted(m) == n) {
				...
				if(norm > 1.e-5) {
					if(p0 > 0.) {
						for(int h=0; h<width[1]; h++) {
							for(int w=0; w<width[0]; w++) {
								value = tmp_data(w, h) / norm;
								bin_data(w, h) += log(value + p0 * exp(-value/p0));
							}
						}
					} else {
						for(int h=0; h<width[1]; h++) {
							for(int w=0; w<width[0]; w++) {
								bin_data(w, h) += tmp_data(w, h) / norm;
							}
						}
					}
				} else {
		...
		return_val = true;
	"""
	read_success = weave.inline(code,
	                            ['fname', 'tmp_data', 'bin_data', 'sel_sorted',
	                             'N_files_sel', 'obj_id', 'lb', 'N_empty', 'p0'],
	                            headers=['<iostream>', '<fstream>', '<stdint.h>', '<math.h>'],
	...
	if verbose:
		print 'Empty stellar surfaces: %d of %d' % (N_empty, N_files_sel)
